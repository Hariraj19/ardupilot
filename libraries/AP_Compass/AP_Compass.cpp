...
void Compass::_reorder_compass_params()
{
    ...
    for(Priority i(0); i<COMPASS_MAX_INSTANCES; i++) {
        ...
        if (curr_state_id != COMPASS_MAX_INSTANCES && int(curr_state_id) != int(i)) {
            //let's swap
            swap_state.copy_from(_state[curr_state_id]);
            _state[curr_state_id].copy_from(_state[StateIndex(int(i))]);
            _state[StateIndex(int(i))].copy_from(swap_state);
        }
    }
}
...
bool Compass::configured(char *failure_msg, uint8_t failure_msg_len)
{
    ...
    for (Priority i(0); i<COMPASS_MAX_INSTANCES; i++) {
        if (_priority_did_list[i] != 0 && use_for_yaw(int(i))) {
            if (!_get_state(i).registered) {
                snprintf(failure_msg, failure_msg_len, "Compass %d not Found", int(i));
                return false;
            }
